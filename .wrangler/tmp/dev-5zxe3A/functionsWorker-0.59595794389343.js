...
// .wrangler/tmp/pages-aPyi9e/functionsWorker-0.59595794389343.mjs
...
function isAuthorized2(request) {
  const authHeader = request.headers.get("authorization");
  if (!authHeader || !authHeader.startsWith("Bearer ")) return false;
  const token = authHeader.slice(7);
  try {
...
__name(isAuthorized2, "isAuthorized2");
__name2(isAuthorized2, "isAuthorized");
...
async function onRequestPost2(context) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ success: false, error: "Contrase\xF1a incorrecta" }),
      { status: 401, headers: { "content-type": "application/json" } }
    );
  } catch (error) {
    return new Response(
      JSON.stringify({ success: false, error: "Error en la petici\xF3n" }),
...
function isAuthorized3(request) {
  const authHeader = request.headers.get("authorization");
  if (!authHeader || !authHeader.startsWith("Bearer ")) return false;
  const token = authHeader.slice(7);
  try {
...
__name(isAuthorized3, "isAuthorized3");
__name2(isAuthorized3, "isAuthorized");
async function onRequestPost3(context) {
  const { request, env } = context;
  if (!isAuthorized3(request)) {
    return new Response(JSON.stringify({ error: "No autorizado" }), { status: 401, headers: { "content-type": "application/json" } });
  }
  try {
    ...
    if (!file) {
      return new Response(JSON.stringify({ error: "No se envi\xF3 ning\xFAn archivo" }), { status: 400, headers: { "content-type": "application/json" } });
    }
...
// .wrangler/tmp/bundle-en07nm/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-en07nm/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.59595794389343.js.map
